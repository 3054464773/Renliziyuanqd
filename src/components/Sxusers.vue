...
//双向绑定
function  zhuanmingzi(data){
  dialogFormVisible3.value=true
  insers2.ybh=data.ybh
  insers2.rzname=data.rzname
...
<template >
  ...
  <div>
    ...
    />
    <el-button style="position: relative;right: -166px;" @click="mohuchaxunxx">查询</el-button>
    <el-input v-model="data.rzname" placeholder="请输入姓名" clearable style="width: 200px;position: relative;right: 105px;" />
    <el-select v-model="data.rzsexxx"  @change="mohuchaxunxx" placeholder="性别" style="position: relative;right: 10px;"  clearable >
      <el-option label="男"  value="男" />
      <el-option label="女"   value="女" />

    </el-select>
    <span style="position: relative;right: -10px;">部门:</span>  <el-select  style="position: relative;right: -15px;"  placeholder="部门"  v-model="data.bbbmmm"  @change="bumenccxx" clearable>
...
<!--      <el-table-column prop="ybh"   label="员工编号" />-->
   ...
   mianshijilu(scope.row.ybh),kaoqin(scope.row.ybh),xinzi(scope.row.ybh),jixiao(scope.row.ybh)">更多</el-button>
    ...
    <el-dialog v-model="dialogFormVisible3" title="转正申请" :modal="insers2">
      ...
      <template #footer>
			<span class="dialog-footer">
				...
				<el-button type="primary" @click="dialogFormVisible3 = false,zzsq(insers2)">确定</el-button>
			</span>
