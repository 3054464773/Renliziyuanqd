...
function mohuchaxun(){
  axios.get("/mohuguizhan?page=1&rzname="+data.rzname).then(function(response){
...
function xinzeng(){
  axios.post("/insertmsgz",insers).then(function(response){
    sx()
    insers.mbh=response.data.Recruit.mbh
    insers.ybh=response.data.Recruit.ybh
    insers.zwbh=response.data.Recruit.zwbh
    insers.mmc=response.data.Recruit.mmc
    insers.ybh1=response.data.Recruit.ybh1
    insers.mzwt=response.data.Recruit.mzwt
...
const rules=reactive({
  ...
  ybh1:[
    {required:true,message:'请输入制作人编号',trigger:'blur'}
  ],
  mzwt:[
    {required:true,message:'请输入面试问题',trigger:'blur'}
  ],
  ybh:[
    {required:true,message:'请输入制作人员编号',trigger:'blur'}
  ]
    ...
    <el-table :data="data.Recruit" style="width: 100%" height="400">
      ...
      <el-table-column label="操作"  width="100">
        <template #default=scope v-slot="scope">
          <el-button size="10px" type="success" plain  @click="dialogTableVisible777=true">新增</el-button><!-- 删除 -->
        </template>
      </el-table-column>
...
<!--          dialogFormVisible=true,-->
  ...
  <el-dialog v-model="dialogTableVisible777" title="新增" >
    <el-form    :model="insers2"  :rules="rules">
      <el-row>
          <el-form-item label="规章编号:" prop="mmc">
            <el-input v-model="insers2.mbh" style="width: 200px;" clearable/>
          </el-form-item>
        <el-col :span="11">
        <el-form-item label="制作人编号" prop="ybh1">
          <el-input v-model="insers2.ybh" style="width: 200px;"  maxlength="11"/>
        </el-form-item>
        </el-col>
          <el-form-item label="面试问题:" prop="mzwt">
            <el-input v-model="insers2.mzwt" style="width: 200px;" clearable/>
          </el-form-item>
      </el-row>
    ...
    <template #footer>
	  	<span class="dialog-footer">
	  		...
	  		<el-button type="primary" @click="dialogTableVisible777 = false,xinzeng2()">确定</el-button>
	  	</span>
  ...
  <el-dialog v-model="dialogFormVisible2" title="新增" >
    <el-form    :model="insers"  :rules="rules">
      <el-row>
        <el-col :span="11">
          <el-form-item label="制定人员编号:" prop="ybh">
        ...
        <el-form-item label="制作人编号" prop="ybh1">
          <el-input v-model="insers.ybh1" style="width: 200px;"  maxlength="11"/>
        </el-form-item>

        <el-col :span="11">
          <el-form-item label="面试问题:" prop="mzwt">
            <el-input v-model="insers.mzwt" style="width: 200px;" clearable/>
          </el-form-item>
